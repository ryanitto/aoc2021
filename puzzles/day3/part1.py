...
def run():
    ...
    columns = list(zip(*lines_to_num))
    gamma = int(''.join(list(map(lambda x: str(int(x.count('0') < x.count('1'))), columns))), 2)
    epsilon = int(''.join(list(map(lambda x: str(int(x.count('0') > x.count('1'))), columns))), 2)
